...
model = keras.Model(inputs=inputs, outputs=outputs, name="VGG11_model_hvd")
model.compile(loss='binary_crossentropy', optimizer=opt,metrics=['accuracy'])
if hvd.rank() == 0:
    print(model.summary())
vgg11_hvd_name = model.name
...
# log messages
if hvd.rank() == 0:
    logdir = os.path.join(os.getcwd(), "logs", "cats-or-dogs"+vgg11_hvd_name+"-reuse-"+
...
if hvd.rank() == 0:
    fname = 'cat-or-dog-' + vgg11_hvd_name + '-reuse.h5'
